    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between gap-2 p-5 border-b border-gray-100 dark:border-gray-800">
          ...
          <button
            onClick={onClose}
            className="text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded p-1 transition-colors flex-shrink-0"
            aria-label="Close"
          >
            ✕
          </button>
          ...
          {events.map((event) => (
            <div key={event.id} className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {event.note && (
                  <p className="text-xs text-gray-500 dark:text-gray-400 italic mt-0.5">{event.note}</p>
                )}
              </div>
              {event.feed === 'idea' && (
                <button
                  onClick={() => onEditIdea(event.id)}
                  className="text-xs text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 flex-shrink-0 transition-colors"
                >
                  Edit
                </button>
              )}
