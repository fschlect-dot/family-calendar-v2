...
const CHIP_STYLES: Record<string, string> = {
  ...
  fred_outlook:     'bg-slate-800 text-white',
  charissa_custody: 'bg-green-600 text-white',
  idea:             'bg-amber-100 text-amber-700 border border-dashed border-amber-500',
};

export default function EventChip({ event, onClick, size = 'sm' }: Props) {
  const base   = CHIP_STYLES[event.feed] ?? 'bg-gray-200 text-gray-700';
