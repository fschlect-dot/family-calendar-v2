...
interface Props {
  initialDate?: string;   // YYYY-MM-DD — pre-fill when opened from a day cell
  idea?: Idea;            // pre-fill when editing existing
...
export default function IdeaModal({ initialDate, idea, onSave, onDelete, onClose }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      await onSave(title.trim(), date, note.trim());
      onClose();
    } finally {
      setBusy(false);
    }
    ...
    >
      ...
      >
        <div className="p-5 border-b border-gray-100">
          <h2 className="font-bold text-gray-900">{idea ? 'Edit Idea' : 'Add Idea'}</h2>
        </div>
          ...
          <div className="flex items-center gap-2 pt-1">
            {onDelete && (
              <button
                ...
                disabled={busy}
                className="mr-auto text-sm text-red-500 border border-red-200 rounded-lg px-3 py-2 hover:bg-red-50 transition-colors disabled:opacity-50"
              >
                Delete
              </button>
            )}
